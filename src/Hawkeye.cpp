...
void drawTag(Mat &frame, const Mat &rvec, const Mat &tvec, const Mat &cameraMatrix, const Mat &distCoeffs,
             const vector<Point3d> &singleObjectPoints, const vector<Point3d> &singleObjectPoints3d) {
...
int main() {
    ...
    RobotTracker tracker(markerLengthMeters, Now());
    Mat markerImage;
    ...
    while (enabledSub.Get() == true) {
        ...
        tracker.setTimestamp(static_cast<int>(Now()));
        odom.getUpdate(tracker.getTimestamps(), [&tracker](const Pose3 *pose, int time) {
            tracker.addOdoFactor(pose, time);
        });
        if (!markerIds.empty()) // if tags are detected
        {
            if (tracker.tagMap.targets.empty()) // if no targets have been detected yet
            {
                ...
                solvePnP(singleObjectPoints, newCorners, cameraMatrix, distCoeffs, rotationVectorOutput,
                         translationVectorOutput, false);
                ...
                if (imagePoints.size() % 4 == 0 && !imagePoints.empty()) {
                    ...
                    solvePnP(objectPoints, imagePoints, cameraMatrix, distCoeffs, rotationVectorOutput,
                             translationVectorOutput, false, SOLVEPNP_SQPNP);
                    ...
                    for (pair<int, vector<Point2f> > data: singleImageMap) {
                        ...
                        if (data.second.size() == 4) {
                            solvePnP(singleObjectPoints, data.second, cameraMatrix, distCoeffs, rotationVectorOutput,
                                     translationVectorOutput, false, SOLVEPNP_IPPE_SQUARE);
                    ...
                    if (unknownCount > 0) {
                        solvePnP(singleObjectPoints, unknownImagePoints, cameraMatrix, distCoeffs, rotationVectorOutput,
                                 translationVectorOutput, false, SOLVEPNP_IPPE_SQUARE);
                        Pose3 robotToTag = tvecrvecToPose(translationVectorOutput, rotationVectorOutput);
        ...
        double pose[3]{
            tracker.getPose().x(), tracker.getPose().y(), tracker.getPose().rotation().yaw()
        };
        posePub.Set(pose);
